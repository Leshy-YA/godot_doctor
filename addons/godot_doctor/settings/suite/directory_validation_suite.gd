...
func _list_files(directory_path: String) -> Array[String] :
	...
	if DirAccess.dir_exists_absolute(directory_path) :
		...
		for file_name : String in DirAccess.get_files_at(directory_path) :
			...
			if file_type == FileType.SCENE and (file_name.ends_with(".tscn") or file_name.ends_with(".scn")) :
				files.append(directory_path + "/" + file_name)
			elif file_type == FileType.RESOURCE and (file_name.ends_with(".tres") or file_name.ends_with(".res")) :
				files.append(directory_path + "/" + file_name)
