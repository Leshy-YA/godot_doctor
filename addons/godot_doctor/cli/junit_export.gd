...
func _generate_junit_xml() -> String:
	...
	xml += '<testsuites name="Godot Doctor Validation" failures="' + str(_fail_count) +'" tests="' + str(_validation_resutls.size()) +'" time="' + str((float(_total_time)* 0.000001)) +'">\n'
	...
	for classname : String in suites.keys() :
		...
		xml += "    <testsuite name=\"" + classname + "\" tests=\"" + str(validation_count) + "\" failures=\"" + str(failures) + "\" skipped=\"" + str(skipped) + "\" time=\"" + str(total_time) + "\">\n"

		# Add validation data.
		for validation in validations:
			
			# Add base validation info.
			xml += "        <testcase name=\"" + validation["name"] + "\" classname=\"" + classname + "\" time=\"" + str(validation["time"]) + "\">\n"
			
			# Add any messages if failed/ignored.
			if validation["status"] == Status.FAIL:
				xml += "            <failure message=\"" + _xml_escape(validation["message"]) + "\">" + _xml_escape(validation["details"]) + "</failure>\n"
			elif validation["status"] == Status.IGNORE:
				xml += "            <skipped message=\"" + _xml_escape(validation["message"]) + "\">" + _xml_escape(validation["details"]) + "</skipped>\n"
