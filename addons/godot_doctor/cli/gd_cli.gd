...
func _ready() -> void:
	...
	if not _cli_validation_settings.report_location.is_empty():
...
## Returns the [CLIValidationSettings] for this validation run. Will load the file from the
## commanline user arguments if supplied, and from the default location set in general
## Godot Doctor settings if not.
func _get_settings() -> CLIValidationSettings:
			...
			# If the resource is not of the right type, fail.
				...
				push_error(
					(
						"Setting resource "
						+ last_arg
						+ " is not of type CLIValidationSettings, but of type "
						+ resource.get_class()
						+ "."
					)
				)
...
func _process_results(object_name: String) -> void:
	...
	if _export != null:
		_export.add_result(
			...
			results,
			_output._message,
			JUnitExport.Status.PASS if passed else JUnitExport.Status.FAIL
		)
